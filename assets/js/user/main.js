...
$('.designer_add').on('click', function() {		
	if ($('.designer_compn').attr('data-sel') != 1) {
		if ($('.designer_compn').attr('data-sel') != 1) mess('Введите свой данный')
	} else {
		$.ajax({
			url: "/user/get.php?designer_add",
			type: "POST",
			dataType: "html",
			data: ({
				id: $('.designer_add').attr('data-id'),
			...
			success: function(data){
				...
				} else if (data == 'yes') mess('Запрос уже отпавлена!');
