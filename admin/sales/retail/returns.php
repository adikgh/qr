<?php include "../../config/core.php";
	...
	// filter cours
	if ($_GET['on'] == 1) $orders = db::query("select * from retail_returns where returns = 1 order by upd_dt desc limit $page_start, $page_age");
	elseif ($_GET['off'] == 1) $orders = db::query("select * from retail_returns where returns = 1 order by upd_dt desc limit $page_start, $page_age");
	else $orders = db::query("select * from retail_returns where returns = 1 order by upd_dt desc limit $page_start, $page_age");


	// site setting
	$menu_name = 'sales';
	$pod_menu_name = 'returns';
