<? include "../../config/core.php";
	...
	else $orders_all = db::query("select * from retail_orders where paid = 1");
	...
	// filter cours
	if ($_GET['on'] == 1) $orders = db::query("select * from retail_orders where paid = 1 order by upd_dt desc limit $page_start, $page_age");
	elseif ($_GET['off'] == 1) $orders = db::query("select * from retail_orders where paid = 1 order by upd_dt desc limit $page_start, $page_age");
	else $orders = db::query("select * from retail_orders where paid = 1 order by upd_dt desc limit $page_start, $page_age");


	// site setting
	$menu_name = 'sales'; // retail_orders
	$pod_menu_name = 'retail'; // retail_orders
						...
						<? while ($buy_d = mysqli_fetch_assoc($orders)): ?>
							...
							<div class="uc_ui">
								<div class="uc_uil">
									...
									<div class="uc_uin_other"><?=$buy_d['total']?> тг</div>
