<?php include "../../config/core.php";
	...
	// filter user all
	if ($_GET['on'] == 1) $users_all = db::query("select * from user");
	elseif ($_GET['off'] == 1) $users_all = db::query("select * from user");
	else $users_all = db::query("select * from user");
	...
	// filter cours
	if ($_GET['on'] == 1) $users = db::query("select * from user order by ins_dt desc limit $page_start, $page_age");
	elseif ($_GET['off'] == 1) $users = db::query("select * from user order by ins_dt desc limit $page_start, $page_age");
	else $users = db::query("select * from user order by ins_dt desc limit $page_start, $page_age");
